 ...
 * 		--------------------------------------------------
 * 
 *		Extend example to include a “Track” case in "active" method of the “Sender” class
 * 
 *		This case is responsible to keep count of amount status was printed to screen, to
 *		be used in the “c @ Continue” case (in first “if” statement) and the “Done” case
 *		of the same method to display a summary.
 * 
 * 		Insertions at rows: 79, 102, 141, 184, 218.
 */
...
object Sender {
	...
	//
	// Added as per requirements to keep count of status messages
	//
	case class Track(finished: Boolean) extends Echo
	// true  - finished running - invoked at: Done case
	// false - continue running - invoked at: c @ Continue case
	//
...
class Sender(path: String, totalMessages: Int, burstSize: Int, payloadSize: Int) extends Actor {
	...
	//
	var currentCount = 0      // Declared variable to keep count of status messages
	//
    ...
    // Added as per requirements
		//
		case Track(finished) =>
		  if (finished == false)                          // This message will be printed after every status message
			{
        currentCount = currentCount + 1
        println(s"Status messages so far: $currentCount")
			}
			else                                            // This message will be printed at the end of the program ONLY 
			{  println(s"Summary: $currentCount status messages in total!\n") }
		//
		...
		case c @ Continue(remaining, t0, t1, n) =>
			...
			if (duration >= 500) {
			  ...
			  //  Added as per requirements
			  //
				actor ! Track(false)   //  Show Message + status message count after each turn
				//
		...
		case Done =>
			...
			//  Added as per requirements
			//
			actor ! Track(true)      // Summary of status message count
			//
